...
# CMC_API_KEY opzionale: se manca i volumi 24h saranno 'n/d'
...
def main_loop():
    while True:
        try:
            ...
            if code != 451:  # silenzia i 451 (blocco Binance)
                try: send_telegram(f"⚠️ Errore dati: HTTP {code or ''}".strip())
                except Exception: pass
        except Exception:
            # silenzioso: niente allegati JSON
            pass
