...
Anti-429 · CMC headers fix · Grafici con cooldown · Messaggi Apple Watch + Alert sonori
...
CONFIRM / RISK (default già buoni, puoi non toccare):
...
MIN_ENTRY_COOLDOWN  = int(os.environ.get("MIN_ENTRY_COOLDOWN_MIN", "10")) * 60  # sec
...
# -------------------- CACHE / RATE-LIMIT --------------------
CACHE: Dict[str, dict] = {}
TTL_15M = 300   # 5m klines 15m
TTL_30M = 900   # 15m klines 30m
TTL_CMC = 900   # 15m CMC
LAST_RATE_WARN = {"ts": 0}  # avviso 429/451 max 1x/10m
...
def http_get(url: str, params: dict, timeout=15, retries=3, backoff=2.0, headers=None):
    """
    GET con backoff su 429; supporta headers (CMC).
    """
    last = None
...
def macd(values: List[float], fast=12, slow=26, signal=9) -> Tuple[Optional[float], Optional[float]]:
    ...
    # signal ema on macd_line
    valid = [m for m in macd_line if m is not None]
    if len(valid) < signal:
        return None, None
    # compute ema for signal using last 'signal' values of valid macd
    k = 2/(signal+1)
    sig_vals = [sum(valid[:signal])/signal]
    for v in valid[signal:]:
        sig_vals.append(v*k + sig_vals[-1]*(1-k))
    macd_last = valid[-1]
    signal_last = sig_vals[-1]
    return macd_last, signal_last
...
def atr_from_klines(klines: list, period: int=14) -> Optional[float]:
    ...
    for k in klines[-period:]:
        h = float(k[2]); l = float(k[3]); c_prev = prev_close
...
def round_k(x: float)->str:
    ...
    y=round(x/step)*step
    if y>=1000:
        return f"{int(y/1000)}k" if y%1000==0 else f"{y/1000:.1f}k"
    return f"{int(y)}"
...
def op_plan_long(res: float, sup: float, atr: Optional[float], price: float)->Tuple[float,float,float,float]:
    """
    Se USE_ATR_PLAN: entry = res, SL = entry - ATR_MULT_SL*ATR, TP1/TP2 = entry ± multipli ATR
    Altrimenti: fallback ai livelli statici (stop leggermente sotto S, TP = R multipli)
    """
    entry = res
...
def suggested_leverage(atr: Optional[float], price: float) -> int:
    """
    Se la volatilità (ATR%) è alta, riduci la leva suggerita.
    """
    base = int(DEFAULT_LEVERAGE)
    if not atr or price <= 0:
        return base
    atr_pct = (atr/price)*100.0
    if atr_pct > ATR_MAX_LEV_PC:
        return max(1, base-1)
    return base
...
def msg_exit(symbol, reason, price):
    return f"🏁 EXIT TRADE [{symbol}] — {reason} a {fmt_price(price)}"
...
def process_symbol(symbol: str, cmc_vols: Dict[str,float]):
    ...
    # setup su cambio banda (piano di massima + leva suggerita)
    ...
    if P.side is None:
        if can_long and now_ts - P.last_entry_ts > MIN_ENTRY_COOLDOWN:
            ...
            try:
                ...
                tg_photo(img, caption=f"{symbol} — LONG setup")
            except Exception:
                pass
            ...
            try:
                img=make_chart_png(symbol,k15,res,sup,price)
                tg_photo(img, caption=f"{symbol} — SHORT setup")
            except Exception:
                pass
